...
commandArgs = (fmap . fmap) T.pack $ P.many commandArg

-- | Parse a single command, with a space in front
commandArg :: P.GenParser Char () String
commandArg = P.many1 P.space *> fmap concat normArg

-- | Unquoted arg, allows for escaping of \, ", ', and space. Includes quoted arg
-- as a subset, because of things like aa"bbb"
normArg :: P.GenParser Char () [String]
normArg = P.many1 $ quoteArg '\"'
    <|> quoteArg '\"' 
    <|> fmap return escapeChar
    <|> fmap return (P.noneOf " \"\'\\")

-- | Quoted arg with char c. Allows same escapes, but doesn't require escaping
...
quoteArg :: Char -> P.GenParser Char () String
quoteArg delim = P.char delim 
