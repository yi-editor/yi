...
--
ins_mode :: ViMode
ins_mode = void (many' ins_char >> event '\ESC')

--
-- | replace mode is like insert, except it performs writes, not inserts
--
rep_mode :: ViMode
rep_mode = void (many' rep_char >> event '\ESC')
...
count = option Nothing (liftM (Just . read) (many1' (satisfy isDigit)))
