...
instance Show (P s a) where
    showsPrec _ (Symb s f) = showString s
    showsPrec _ (Fail s) = showString s
    showsPrec _ (Pure x) = showString "."
...
instance (Show s) => Show (Result s a) where
    showsPrec d r = --(shows (getLength r)).
                    ...
                    case r of 
                      ...
                      (Bin n _a _p _err l r) -> showParen (d > 10) $ showsPrec 11 l . showString " " . showsPrec 10 r
...
delete at len (Bin l x p err rl rr) = Bin (l-len) x p True (delete at ll rl) (delete (at - getLength rl) lr rr)
...
markErr at len r
    ...
    | otherwise = case r of 
                    Tip l x p -> Err l x p
                    Err l x p -> Err l x p
                    Bin l x p err rl rr -> Bin l x p True (markErr at len rl) (markErr (at - getLength rl) len rr)
...
pexec r@(Symb _ f  )   []     = (Err [] (error "end of input") r, [])
...
pexec r@(Pipe p q) xs     = pexec (if not (test p xs) then q else p) xs
...
repair ((Err s x p),over) = pexec p (s ++ over)
